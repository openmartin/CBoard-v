...
let options = {
  ...
  methods: {
    initByWidget(reload) {
      ...
      this.loading = true;
      this.$store
        .dispatch("dashboard/getWidgetData", {
          widgetData: this.widgetData,
          filters: this.filters,
          reload: reload,
          config: this.widget.widget.data.config
        })
        .then(seriesData => {
          this.loading = false;
          this.$nextTick(() => {
    ...
    createOption(seriesData) {
      ...
      let option = {
        ...
        legend: {
          left: "left",
          orient: "vertical",
          data: parseLegendData(seriesData.groups, seriesData.values)
        },
        radar: {
          //shape: 'circle',
          name: {
      ...
      if (styleOption.legend) {
        for (let prop in styleOption.legend) {
          if (styleOption.legend[prop] !== "")
            option.legend[prop] = styleOption.legend[prop];
        }
      }
      ...
      function parseLegendData(groups, values) {
        let legendData = [];
        groups.forEach(group => {
          values.forEach(value => {
            legendData.push(group + "-" + value.name);
          });
        });
        return legendData;
      }
      ...
      function parseSeriesData(groups, data, values, keys) {
        let arr = [];
        groups.forEach(group => {
          values.forEach(value => {
            let item = {
            ...
            keys.forEach(key => {
              let itemValue =
                data[group][value.name][value.aggType][key.join("-")];
